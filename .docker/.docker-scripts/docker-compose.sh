...
if [ "${COMMAND}" == "push" ]; then
    ...
    docker compose --project-directory . -f ".docker/docker-compose.${VARIANT}.yaml" run "traefik" "${RUN_COMMAND}"
else
    docker compose --project-directory . -f ".docker/docker-compose.${VARIANT}.yaml" --profile core "${COMMAND}"
fi
